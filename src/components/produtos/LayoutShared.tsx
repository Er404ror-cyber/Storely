...
export const ProductImage = memo(function ProductImage({
  ...
  const imageSrc = src || IMAGE_FALLBACK;
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-zinc-100 dark:bg-zinc-900" style={{ contain: "layout paint" }}>
      {/* Skeleton Shimmer de Fundo fixo */}
      <div
        className={`absolute inset-0 z-0 animate-pulse transition-opacity duration-500 ease-out ${
          isLoaded ? "opacity-0 pointer-events-none" : "opacity-100 bg-zinc-200 dark:bg-zinc-800"
        }`}
      />
      ...
      <img
        ...
        className={`h-full w-full object-cover transition-all duration-500 ease-out hover:saturate-125 transform-gpu ${
    ...
    >
      <span
        className={[
          ...
          isLg
            ? "max-w-[80px] text-[10px] md:max-w-[160px] md:text-[11px]"
            : "max-w-[55px] text-[8px] md:max-w-[110px] md:text-[10px]",
        ].join(" ")}
        title={name}
      >
        {name}
      ...
      <div className="ml-1.5 flex min-w-0 shrink-0 items-center md:ml-2">
        ...
        <span
          className={[
            "shrink-0 truncate rounded-full px-1.5 py-0.5 text-[8px] font-black leading-none text-white md:px-2 md:py-1 md:text-[10px]",
            product.has_discount ? "bg-rose-600" : "bg-blue-600",
            isLg ? "max-w-[80px] sm:max-w-[100px]" : "max-w-[60px] sm:max-w-[85px]",
          ].join(" ")}
        >
...
export const ModernScrollRow = memo(function ModernScrollRow({
  ...
  return (
    <div className="relative w-full overflow-hidden group/row" style={{ contain: "layout paint" }}>
      <div
        ref={scrollRef}
      ...
      {canScrollLeft && (
        ...
        >
          <ChevronLeft size={36} className="drop-shadow-sm transition-transform duration-300 hover:-translate-x-1" />
        </button>
      )}
      ...
      {canScrollRight && (
        ...
        >
          <ChevronRight size={36} className="drop-shadow-sm transition-transform duration-300 hover:translate-x-1" />
        </button>
      )}
    </div>
  );
