...
export const StorageDashboard: React.FC<StorageDashboardProps> = ({ 
  ...
  const { totalWeightMB, isOverTotalLimit, hasPendingUploads, isAtLimit } = stats;
    ...
    <div className="mb-6 space-y-2">
      <div className={`p-4 rounded-2xl border transition-all flex flex-wrap items-center justify-between gap-4 ${
        ...
        <div className="flex items-center gap-6">
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-sm font-black transition-colors ${
                isOverTotalLimit ? 'text-red-600 dark:text-red-50' : 'text-zinc-900 dark:text-blue-500' 
              }`}>
          ...
          {hasPendingUploads && (
            <div className="flex items-center gap-3 px-3 py-2 bg-white dark:bg-zinc-950 rounded-xl border border-blue-200 shadow-sm">
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-blue-600 uppercase">
                  {isOverTotalLimit ? "Limite Excedido" : "Ação Necessária"}
                </span>
                <span className="text-[11px] text-gray-400 font-bold">
                  {isOverTotalLimit ? "Remova arquivos (Max 15MB)" : "Arquivos locais"}
                </span>
              </div>
              
              <button
                onClick={onSync}
                disabled={isSyncing || isOverTotalLimit}
              ...
              >
                ...
                {isOverTotalLimit ? "Bloqueado" : "Sincronizar"}
              </button>
            </div>
          )}
        </div>
        ...
        >
          {isAtLimit ? <X size={14} /> : '+'} {isAtLimit ? t('gallery_limit_reached') : t('gallery_add')}
        </button>
      </div>
      ...
      {hasPendingUploads && (
        <div className="flex items-center gap-2 px-2 text-blue-600 dark:text-blue-400">
          ...
          <span className="text-[9px] font-black uppercase tracking-widest animate-pulse">
            Sincronize as mídias acima para habilitar o salvamento da página
          </span>
        </div>
      )}
    </div>
  ...
  <div className="relative py-14 md:py-20 mt-4 flex flex-col items-center overflow-hidden">
    ...
    <div className="relative z-10 text-center flex flex-col items-center gap-8">
      ...
      <div className="flex items-center gap-6 md:gap-8 py-3 px-6 md:px-10 rounded-2xl md:rounded-full border border-current/10 ">
        ...
        <div className="flex flex-col items-center">
          <span className="text-[8px] font-black opacity-40 uppercase tracking-tighter">Photos</span>
          <span className="text-[10px] font-bold">Max 1MB</span>
        </div>
        <div className="w-px h-6 bg-current opacity-10" />
        <div className="flex flex-col items-center">
          <span className="text-[8px] font-black opacity-40 uppercase tracking-tighter">Video</span>
          <span className="text-[10px] font-bold">Max 10MB</span>
        </div>
      </div>
